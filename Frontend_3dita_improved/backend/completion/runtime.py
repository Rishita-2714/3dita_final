...
def _make_local_completion(point_cloud, profile: str, params: Dict[str, Any]):
    ...
    # Use a low threshold (0.08) so only truly planar surfaces (walls, floors)
    # are skipped — temple columns/spires have low flatness (~0.05-0.15) due
    # to their elongated shape and must NOT be excluded from completion.
    if flatness < 0.08 and not params.get("force_completion"):
        return None, {
...
def _merge_completion(point_cloud, generated_cloud, profile: str, params: Dict[str, Any], metadata: Dict[str, Any]):
    ...
    if metadata.get("completion_model") == "local_hybrid_surface":
        max_added_ratio = 1.2 if profile == "hq" else 1.0 if profile == "balanced" else 0.82
    elif metadata.get("completion_model") == "local_cylindrical_surface":
        max_added_ratio = 0.9 if profile == "hq" else 0.72 if profile == "balanced" else 0.58
    elif metadata.get("completion_model") == "local_rotational_symmetry":
        max_added_ratio = 1.05 if profile == "hq" else 0.86 if profile == "balanced" else 0.68
    else:
        max_added_ratio = 0.65 if profile == "hq" else 0.5 if profile == "balanced" else 0.35
